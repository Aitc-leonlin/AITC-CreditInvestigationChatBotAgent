...
class OrganizationRepository:
    ...
    def list_data_policies(self, *, subject_type: str = "", subject_id: str = "", resource_code: str = "") -> list[dict[str, Any]]:
        ...
        params: list[Any] = []
        if subject_type:
            clauses.append("p.subject_type = ?")
            params.append(subject_type)
        if subject_id:
            clauses.append("p.subject_id = ?")
            params.append(subject_id)
        if resource_code:
            clauses.append("p.resource_code = ?")
        ...
        with get_membership_connection() as connection:
            rows = connection.execute(
                f"""
                SELECT p.*,
                       COALESCE(r.name, u.display_name) AS subject_name
                FROM membership_data_permission_policy p
                LEFT JOIN membership_role r ON r.id = p.subject_id AND p.subject_type = 'ROLE'
                LEFT JOIN membership_user u ON u.id = p.subject_id AND p.subject_type = 'USER'
                WHERE p.deleted_at IS NULL
                {where_sql}
                ORDER BY p.resource_code ASC, p.subject_type ASC, subject_name ASC
                """,
                params,
            ).fetchall()
        return [self.policy_row(row) for row in rows]

    def upsert_data_policy(self, values: dict[str, Any]) -> dict[str, Any]:
        now = utc_now_iso()
        existing = self._find_policy(values["subjectType"], values["subjectId"], values["resourceCode"])
        payload = {
            "subject_type": values.get("subjectType", "ROLE"),
            "subject_id": values["subjectId"],
            "resource_code": values["resourceCode"],
            "data_scope": values.get("dataScope", "ONLY_MYSELF"),
            "custom_scope_json": json.dumps(values.get("customScope", []), ensure_ascii=False),
            "row_rule_json": json.dumps(values.get("rowRule", {}), ensure_ascii=False),
            "field_rule_json": json.dumps(values.get("fieldRule", {}), ensure_ascii=False),
            "masking_rule_json": json.dumps(values.get("maskingRule", {}), ensure_ascii=False),
            "status": values.get("status", "ACTIVE"),
            "updated_at": now,
        }
        with membership_transaction() as connection:
            if existing:
                assignments = ", ".join(f"{column} = ?" for column in payload)
                connection.execute(
                    f"UPDATE membership_data_permission_policy SET {assignments} WHERE id = ?",
                ...
                self._insert(
                    connection,
                    "membership_data_permission_policy",
                    {
                        "id": policy_id,
                        **payload,
                        "created_at": now,
        ...
        return self.list_data_policies(subject_type=values["subjectType"], subject_id=values["subjectId"], resource_code=values["resourceCode"])[0]

    def delete_data_policy(self, policy_id: str) -> bool:
        return self._soft_delete("membership_data_permission_policy", policy_id)

    def list_row_rules(self, policy_id: str = "") -> list[dict[str, Any]]:
        return self._list_policy_children(
            "membership_row_permission_rule",
            policy_id,
            self.row_rule_row,
            "rule_name ASC",
        )

    def create_row_rule(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = {
            ...
            "expression_json": json.dumps(values.get("expression", {}), ensure_ascii=False),
            "effect": values.get("effect", "ALLOW"),
            "status": values.get("status", "ACTIVE"),
            "created_at": now,
            "updated_at": now,
            "deleted_at": None,
        ...
        with membership_transaction() as connection:
            self._insert(connection, "membership_row_permission_rule", payload)
        return self.list_row_rules(policy_id=values["policyId"])[-1]

    def delete_row_rule(self, rule_id: str) -> bool:
        return self._soft_delete("membership_row_permission_rule", rule_id)

    def list_field_rules(self, policy_id: str = "") -> list[dict[str, Any]]:
        return self._list_policy_children(
            "membership_field_permission_rule",
            policy_id,
            self.field_rule_row,
            "field_name ASC",
        )

    def upsert_field_rule(self, values: dict[str, Any]) -> dict[str, Any]:
        rule_id = str(uuid.uuid4())
        now = utc_now_iso()
        payload = {
            "id": rule_id,
            ...
            "can_read": 1 if values.get("canRead", True) else 0,
            "can_write": 1 if values.get("canWrite") else 0,
            "status": values.get("status", "ACTIVE"),
            "created_at": now,
            "updated_at": now,
            "deleted_at": None,
        ...
        with membership_transaction() as connection:
            self._insert(connection, "membership_field_permission_rule", payload)
        return self.list_field_rules(policy_id=values["policyId"])[-1]

    def delete_field_rule(self, rule_id: str) -> bool:
        return self._soft_delete("membership_field_permission_rule", rule_id)

    def list_masking_rules(self, policy_id: str = "") -> list[dict[str, Any]]:
        return self._list_policy_children(
            "membership_sensitive_data_masking_rule",
            policy_id,
            self.masking_rule_row,
            "field_name ASC",
        )

    def upsert_masking_rule(self, values: dict[str, Any]) -> dict[str, Any]:
        rule_id = str(uuid.uuid4())
        now = utc_now_iso()
        payload = {
            "id": rule_id,
            ...
            "resource_code": values["resourceCode"],
            "field_name": values["fieldName"],
            "masking_type": values.get("maskingType", "PARTIAL"),
            "masking_pattern": values.get("maskingPattern", ""),
            "status": values.get("status", "ACTIVE"),
            "created_at": now,
            "updated_at": now,
            "deleted_at": None,
        ...
        with membership_transaction() as connection:
            self._insert(connection, "membership_sensitive_data_masking_rule", payload)
        return self.list_masking_rules(policy_id=values["policyId"])[-1]

    def delete_masking_rule(self, rule_id: str) -> bool:
        return self._soft_delete("membership_sensitive_data_masking_rule", rule_id)
    ...
    def policy_row(self, row: sqlite3.Row) -> dict[str, Any]:
        return {
            "id": row["id"],
            ...
            "customScope": self._json(row["custom_scope_json"], []),
            "rowRule": self._json(row["row_rule_json"], {}),
            "fieldRule": self._json(row["field_rule_json"], {}),
            "maskingRule": self._json(row["masking_rule_json"], {}),
            "status": row["status"],
            "createdAt": row["created_at"],
            "updatedAt": row["updated_at"],
        }

    def row_rule_row(self, row: sqlite3.Row) -> dict[str, Any]:
        return {
            "id": row["id"],
            "policyId": row["policy_id"],
            "resourceCode": row["resource_code"],
            "ruleName": row["rule_name"],
            "expression": self._json(row["expression_json"], {}),
            "effect": row["effect"],
            "status": row["status"],
            "createdAt": row["created_at"],
            "updatedAt": row["updated_at"],
    ...
    def field_rule_row(self, row: sqlite3.Row) -> dict[str, Any]:
        return {
            "id": row["id"],
            "policyId": row["policy_id"],
            "resourceCode": row["resource_code"],
    ...
    def masking_rule_row(self, row: sqlite3.Row) -> dict[str, Any]:
        return {
            "id": row["id"],
            "policyId": row["policy_id"],
            "resourceCode": row["resource_code"],
            ...
            "maskingPattern": row["masking_pattern"],
            "status": row["status"],
            "createdAt": row["created_at"],
            "updatedAt": row["updated_at"],
        }

    def _find_policy(self, subject_type: str, subject_id: str, resource_code: str) -> dict[str, Any] | None:
        rows = self.list_data_policies(subject_type=subject_type, subject_id=subject_id, resource_code=resource_code)
        return rows[0] if rows else None

    def _list_policy_children(self, table_name: str, policy_id: str, mapper: Any, order_sql: str) -> list[dict[str, Any]]:
        where_sql = "AND policy_id = ?" if policy_id else ""
        params = [policy_id] if policy_id else []
        with get_membership_connection() as connection:
            rows = connection.execute(
                f"""
                SELECT *
